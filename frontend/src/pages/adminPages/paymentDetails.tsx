        ...
        <div className="flex-1 bg-gray-100">
          <div className="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-4 flex-col md:flex-row">
              <button className="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition duration-200 mt-4 md:mt-0" onClick={exportToExcel}>Export to Excel</button>
              <button className="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 transition duration-200 mt-4 md:mt-0" onClick={exportToPDF}>Export to PDF</button>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full table-auto">
                ...
                <tbody>
                  {currentPayments.map((payment: IPayment) => (
                    <tr key={payment.paymentId} className="border-b">
                      ...
                      <td className="p-4 text-gray-700">{payment.date ? new Date(payment.date).toLocaleDateString() : 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
